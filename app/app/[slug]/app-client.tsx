...
export default function AppPageClient() {
  ...
  const { data: salesTransactions, isLoading: salesLoading } = useQuery<
    Transaction[]
  >({
    queryKey: ["sales", slug],
    queryFn: async () => {
  ...
  const { resolvedAddresses: salesResolvedAddresses } =
    useResolveAddresses(salesAddresses);
              ...
              {displayIconUrl ? (
                ...
                <div className="relative h-32 md:h-40 overflow-hidden bg-zinc-900/50">
                  ...
                  <div className="absolute inset-0 flex items-center justify-center">
                    <div
                      className={`w-20 h-20 rounded-2xl overflow-hidden border-2 border-zinc-600 shadow-2xl p-2 ring-4 ring-black/50 ${
                        displayIconNeedsDarkBg ? "bg-zinc-900" : "bg-white"
                      }`}
                    >
                      {/* eslint-disable-next-line @next/next/no-img-element */}
                    ...
                    (!salesTransactions || salesTransactions.length === 0) && (
                      <div className="rounded-xl bg-zinc-950 border border-zinc-800 p-12 text-center">
                        ...
                        <p className="text-zinc-400 text-sm">
                          No purchases have been made for {displayName} invites
                          yet.
                        </p>
                      </div>
                    )}
